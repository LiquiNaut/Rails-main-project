...
def generate_demo_invoices
  categories = {
    'web development' => ['vývoj webovej aplikácie React + Rails', 'frontend optimalizácia Tailwind',
                          'REST API integrácia', 'responsive design mobil/tablet'],
    'mobile development' => ['iOS Swift aplikácia', 'Android Kotlin natívna app', 'React Native cross-platform',
                             'push notifikácie Firebase'],
    'grafický dizajn' => ['logo a firemná identita', 'UI/UX design Figma prototyp', 'print materiály A4/A5',
                          'ikony SVG set'],
    'IT konzultácie' => ['AWS cloud migrácia', 'bezpečnostný audit systému', 'DevOps CI/CD pipeline',
                         'softvérová architektúra'],
    'server admin' => ['Linux server správa Ubuntu', 'Docker kontajner deployment', 'PostgreSQL performance tuning',
                       'automatizované backupy S3'],
    'social media' => ['Facebook Ads kampane', 'Instagram content kalendár', 'LinkedIn B2B lead generation',
                       'social media reporting'],
    'SEO optimalizácia' => ['on-page SEO technické úpravy', 'Google Analytics 4 setup', 'keyword research SEMrush',
                            'schema.org markup'],
    'custom software' => ['ERP systém customizácia', 'CRM HubSpot integrácia', 'automatizácia workflow Zapier',
                          'Electron desktop app'],
    'technická podpora' => ['24/7 SLA support', 'urgent hotfix production', 'bug fixing sprint', 'preventívna údržba'],
    'data analytics' => ['Power BI interaktívne dashboardy', 'SQL business reporty', 'data warehouse Snowflake',
                         'prediktívna analýza']
  }

  # Vytvor demo user (ak neexistuje)
  demo_user = User.find_or_create_by(email: 'demo@financegpt.sk') do |u|
    u.password = 'Demo123!'
    u.password_confirmation = 'Demo123!'
  end

  # Vytvor 12 demo klientov (ak neexistujú)
  client_names = ['IT Solutions s.r.o.', 'Dizajn Studio SK', 'Marketing Agency Bratislava', 'Cloud Experts',
                  'WebDev Freelancer', 'Mobilní Vývojári', 'Grafika Pro', 'DevOps Team', 'SEO Masters', 'Data Analytics Hub', 'SupportLine', 'Software Custom']

  client_names.each do |name|
    Entity.find_or_create_by(entity_name: name) do |e|
      e.ico = "SK#{rand(8**7..9**7)}"
      e.dic = "SK#{rand(9**8..9**9)}"
    end
  end

  entities = Entity.limit(12)
  ...
  categories.each do |category, phrases|
    10.times do |i|
      ...
      note = "#{phrase}. Dummy poznámka."

      invoice = Invoice.create!(
        user_id: demo_user.id,
        invoice_name: "Faktúra #{category.titleize} ##{i + 1}",
        invoice_number: "FAC-#{1000 + created_count}/#{Date.current.year}",
        issue_date: rand(1.year.ago..1.month.ago),
        due_date: rand(30..90).days.since(Date.current),
        vehicle_information: nil, # optional
        ...
        total_price_without_tax: rand(800.0..4500.0).round(2),
        vat_rate_percentage: 20.0,
        total_tax_amount_eur: (0.2 * rand(800.0..4500.0)).round(2)
      )

      if invoice.persisted?
        # Create seller and buyer entities linked to this invoice
        Entity.create!(entity_name: 'Naša Firma s.r.o.', entity_type: 'seller', invoice_id: invoice.id)
        Entity.create!(entity_name: entity.entity_name, entity_type: 'buyer', invoice_id: invoice.id)

        created_count += 1
        print '.'
      else
        puts "\nChyba: #{invoice.errors.full_messages}"
      end
    end
    puts "\n✓ #{category}: 10 faktúr"
  end

  puts "\n✅ HOTOVO! Vytvorených #{created_count} demo faktúr pre user: #{demo_user.email}"
  puts 'Spusti: rails embeddings:backfill pre vektorizáciu'
end
