...
class SqlGeneratorTool < RubyLLM::Tool
  ...
  def self.load_invoice_schema
    ...
    begin
      ...
      schema_string += ActiveRecord::Base.connection.columns('invoices').map do |col|
        "- #{col.name} (#{col.sql_type})"
      end.join("\n")
      schema_string += "\n\n"

      schema_string += "Tabuľka: entities\n"
      schema_string += ActiveRecord::Base.connection.columns('entities').map do |col|
        "- #{col.name} (#{col.sql_type})"
      end.join("\n")
      schema_string += "\n\n"

      schema_string += "Tabuľka: bank_details\n"
      schema_string += ActiveRecord::Base.connection.columns('bank_details').map do |col|
        "- #{col.name} (#{col.sql_type})"
      end.join("\n")
    rescue NameError => e
