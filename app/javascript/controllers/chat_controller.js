...
export default class extends Controller {
  ...
  static values = { chatAskPath: String, chatId: Number }; // Cesta pre fetch request + ID aktívneho chatu
  ...
  sendMessage() {
    ...
    if (!prompt && !this.imageData) {
      ...
      this.inputTarget.placeholder =
        "Prosím, zadajte správu alebo nahrajte obrázok.";
      setTimeout(() => {
    ...
    fetch(this.chatAskPathValue, {
      ...
      .catch((error) => {
        ...
        this.addMessage(
          "system",
          `Nastala chyba pri komunikácii: ${error.message}`,
        );
      })
      .finally(() => {
  ...
  addMessage(sender, text) {
    ...
    if (sender === "user") {
      ...
      // Pre 'system' správy môžeme použiť iný štýl alebo žiadny
